        ...
        for j in range(len(video_frames_all)-self.num_frames + 1):
            ...
            if j == 0:
                if self.num_frames == 1:
                    for i in range(self.num_frames):#
                        ...
                        for tile_i in range(len(current_all_intermediate_embeddings[0])):
                            stacked_two_embeddings_tile_list.append(np.stack([current_all_intermediate_embeddings[i][tile_i], current_all_intermediate_embeddings[i][tile_i]], axis=self.time_stack_axis))
                ...
                if self.num_frames == 1:
                    stacked_two_embeddings_tile_list = []
                    for tile_i in range(len(current_all_intermediate_embeddings[0])):
                        stacked_two_embeddings_tile_list.append(np.stack([current_all_intermediate_embeddings[num_frames-1][tile_i], current_all_intermediate_embeddings[num_frames-1][tile_i]], axis=self.time_stack_axis))
                    instances = self.get_merged_instances(stacked_two_embeddings_tile_list)
                    instance_tracker.add_new_label_image(instances)
                    if self.save_all_embeddings:
                        for tile_i, e in enumerate(stacked_two_embeddings_tile_list):
...
if __name__ == '__main__':
    ...
    model_file_name = "E:/code/data/myfile/01_model/DIC-C2DH-HeLa"
